...
import com.hotel.service.net.module.workguide.WorkGuideApi;
 ...
 */
public class WorkGuideActivity extends BaseFragmentActivity implements PullToRefreshView.OnHeaderRefreshListener, PullToRefreshView.OnFooterRefreshListener {

    private String TAG = WorkGuideActivity.class.getCanonicalName();
    @Inject
    WorkGuideApi workGuideApi;
...
//        activeList.setFocusableInTouchMode(true);
        activeList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ResWorkGuide.WorkGuideInfoList tm = eventAdapter.getItem(position);
                Intent i = new Intent(WorkGuideActivity.this, WorkGuideContentActivity.class);
    ...
    private void getCultureInfo() {
        ...
        subscription.add(workGuideApi.getWorkGuide(
                new ReqWorkGuide.Builder()
                        .setUserId("")
                    ...
                    public void call(Throwable throwable) {
                        if (throwable instanceof RetrofitError) {
                            switch (((RetrofitError) throwable).getKind()) {
                                case NETWORK:
                                    Toast.makeText(WorkGuideActivity.this, "网络问题", Toast.LENGTH_SHORT).show();
                                    break;
                                case CONVERSION:
                                    break;
                                case HTTP:
     ...
     * 更新列表信息
     */
    private void getDateEvent(ResWorkGuide resWorkGuide) {

        totalPage = StringUtil.str2int(resWorkGuide.s_total);
        nowPage = StringUtil.str2int(resWorkGuide.s_num);
        nowPage++;

        if (firstLoad) {
            refreshableView.completeNow();
            eventAdapter.clear();
            eventAdapter.addAll(resWorkGuide.getData());
    ...
    //配置adpter
    public class EventAdapter extends QuickAdapter<ResWorkGuide.WorkGuideInfoList> {

        public EventAdapter() {
            super(WorkGuideActivity.this, R.layout.info_detail_img_item);
        }

        @Override
        protected void convert(BaseAdapterHelper helper, ResWorkGuide.WorkGuideInfoList item) {
